...
set output "test-deconvolution.svg"
plot "test-deconvolution.csv" using (($1)/3600):(1000*($8)) with lines linestyle 4 ti "P_1", \
 "test-deconvolution.csv" using (($1)/3600):(1000*($9)) with lines linestyle 3 ti "P_2", \
 "test-deconvolution.csv" using (($1)/3600):(1000*($10)) with lines linestyle 1 ti "P_{Sum}", \
 "experimental.dat" using (($1)/3600):(1000*($5)) with lines linestyle 2 ti "P_{Exp}"

set output "test-deconvolution-black.svg"
